...
class LogLikelihood:
    ...
    \(\Sigma\), equivalent to ``scipy.linalg.pinv`` on symmetric PSD
    inputs and handling rank-deficient \(\Sigma\) gracefully) and begins
    ...
    **Alignment guarantee.** After the initial warmup (first prediction
    ...
    cadence as** `MinimumVariance`, so corresponding records line up
    element-by-element.  Output is 0 if no target tick was accumulated
    ...
    **Memory.** Both `predictions` and `target` are `Array` inputs -
    ...
    target : Array, shape ``(N,)``
        Live cross-sectional realized target values, produced at every
        tick (e.g. a `PctChange` node).  Non-finite entries contribute
        zero to the Mahalanobis quadratic.

    Parameters
    ----------
    num_stocks
        Cross-section size `N`.
    """
...
def _log_pdet_and_pinv(sigma: np.ndarray) -> tuple[float, np.ndarray]:
    ...
    - the Moore-Penrose pseudo-inverse is
      ...
      ``scipy.linalg.pinv`` on symmetric PSD inputs.
...
def build(**kwargs) -> LogLikelihood:
    """Construct a :class:`LogLikelihood` operator.

    Build kwargs
    ------------
    num_stocks : int
        Cross-section size `N` (predictions are ``(N, N)``, target ``(N,)``).
    """
    return LogLikelihood(num_stocks=int(kwargs["num_stocks"]))
