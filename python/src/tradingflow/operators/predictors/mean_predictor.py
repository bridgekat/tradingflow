...
class MeanPredictor[T](
    ...
    the last `max_periods` aligned feature/target pairs from the upstream
    ...
    those operators convert to linear returns internally via the
    lognormal moment map.
    ...
    at time t predicts the return from t to t+1.
    ...
    refit_every
        ...
        between rebalances (e.g. pooled regressions over all history).
    max_periods
    ...
    min_periods
        ...
        observations across the aligned pairs receive `NaN` in the
    ...
    def compute(
        ...
        assert n_features == n_target, (
            f"mean_predictor: features and target lengths differ, "
