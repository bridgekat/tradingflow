...
def _sym_givens(a: float, b: float) -> Tuple[float, float, float]:
    ...
    if abs(b) > abs(a):
        t = a / b
        s = float(np.sign(b)) / math.sqrt(1.0 + t * t)
        return s * t, s, b / s
    t = b / a
    c = float(np.sign(a)) / math.sqrt(1.0 + t * t)
    return c, c * t, a / c
...
def minres_mnr(
    ...
    if beta1 == 0.0:
        z = np.zeros(n)
        return MinresResult(z, z, z, 0, 0, 9)
    Krhs = op(rhs)
    kmv = 1
    Kn = float(norm(Krhs))
    if Kn == 0.0:
        z = np.zeros(n)
        return MinresResult(z, z, rhs.copy(), kmv, 0, 9)
...
def mnr_solve(
    A_op: LSOperator, b: np.ndarray, E: np.ndarray, f: np.ndarray, rtol: float = 1e-8, maxit: Optional[int] = None
) -> MnrResult:
...
class CompletionInfo:
    ...
    iteration: int = -1
    active_size: int = 0
    inactive_violation: float = np.inf
    eq_proj_residual: float = np.inf
    stationarity: float = np.inf
    min_active_multiplier: float = np.nan
    multiplier_feasible: bool = False
    kmv: int = 0


def completion_certificate(grad: np.ndarray, E: np.ndarray, G_A: np.ndarray, tol: float = 1e-7):
    ...
    if ma == 0:
        if me == 0:
            stat = float(norm(grad, ord=np.inf))
            return stat <= tol, stat, np.nan
        lam, *_ = np.linalg.lstsq(E.T, -grad, rcond=None)
        stat = float(norm(E.T @ lam + grad, ord=np.inf))
        return stat <= tol, stat, np.nan
    ...
    nu = lp.x[me:]
    min_nu = float(np.min(nu)) if ma else np.nan
    return stat <= tol and min_nu >= -tol, stat, min_nu


def active_set_completion(
    A_op: LSOperator,
    b: np.ndarray,
    E: np.ndarray,
    f: np.ndarray,
    ...
    *,
    ...
    for i in active_hi:
        ...
        e[i] = 1.0
        rows.append(e)
        rhs_rows.append(float(upper[i]))
        g_rows.append(e)
    for i in active_lo:
        e = np.zeros(n)
        e[i] = 1.0
        rows.append(e)
        rhs_rows.append(float(lower[i]))
        g_rows.append(-e)
    if rows:
    ...
    comp = CompletionInfo(attempted=True, active_size=int(active_hi.size + active_lo.size))
    ...
    comp.stationarity = stat
    comp.min_active_multiplier = min_nu
    comp.multiplier_feasible = ok
    comp.accepted = comp.eq_proj_residual <= feasibility_tol and comp.inactive_violation <= feasibility_tol and ok
...
def admm_mnr(
    ...
    *,
    ...
    max_outer: int = 400,
    inner_rtol0: float = 1e-4,
    inner_power: float = 1.25,
    inner_floor: float = 1e-8,
    completion: bool = True,
    stable_q: int = 5,
    min_completion_iter: int = 10,
    ...
    for it in range(max_outer):
        ...
        y = Gx + u
        z_new = np.minimum(h, y)
        u = u + Gx - z_new

        rp = float(norm(Gx - z_new))
        rd = float(tau * norm(z_new - z))
        eq = float(norm(E @ x_new - fR)) if me else 0.0
        Ax = A_op.matvec(x_new)
        obj = 0.5 * float(norm(Ax - b) ** 2)
        hist.append(dict(iter=float(it + 1), obj=obj, primal=rp, dual=rd, eq_proj=eq, kmv=float(total_kmv)))
        ...
        if verbose and it % 25 == 0:
            print(f"  it={it:4d} obj={obj:.6f} rp={rp:.2e} rd={rd:.2e} " f"inner={res.kmv} hi={hi.size} lo={lo.size}")

        if completion and it + 1 >= min_completion_iter and stable >= stable_q:
            xc, comp = active_set_completion(
                A_op, b, E, f, lower, upper, lo, hi, feasibility_tol=feasibility_tol, cert_tol=cert_tol
            )
            comp.iteration = it + 1
            total_kmv += comp.kmv
            if xc is not None:
...
def solve_portfolio(
    *,
    ...
    if factor_loadings is not None:
        F = np.asarray(factor_loadings, float)
        n = F.shape[0]
        D = np.full(n, 1e-4) if specific_var is None else np.asarray(specific_var, float)
        A_op = (
            DenseLSOperator(np.vstack([math.sqrt(2 * delta) * F.T, math.sqrt(2 * delta) * np.diag(np.sqrt(D))]))
            if dense
            else FactorLSOperator(F, D, delta)
        )
    ...
    if sector_ids is not None:
        ...
        for j in range(S.shape[0]):
            if sector_targets[j] > 1e-12:
                E_rows.append(S[j])
                f_rows.append(float(sector_targets[j]))
    if redundant_budget:
        E_rows.append(np.ones(n))
        f_rows.append(float(budget))
    E = np.vstack(E_rows)
    f = np.asarray(f_rows)
    ...
    x = res.x
    metrics = dict(
        objective=0.5 * float(x @ sig(x)) - (float(b @ A_op.matvec(x)) if False else 0.0),
        sum_weights=float(x.sum()),
        eq_projected=float(norm(E @ x - row_project_rhs(E, f))),
        min_weight=float(x.min()),
        max_weight=float(x.max()),
        outer_iterations=res.outer_iters,
        K_matvecs=res.total_kmv,
        status=res.status,
        time=res.time,
...
def solve_multi_account(
    *,
    ...
    for j in range(K):
        ...
        r[j * n : (j + 1) * n] = 1.0
        E_rows.append(r)
        f_rows.append(float(budgets[j]))
    if redundant_firm:
        E_rows.append(np.ones(N))
        f_rows.append(float(np.sum(budgets)))
        ...
        for j in range(K):
            for s in range(ns):
                if bs[s] > 1e-12:
                    ...
                    r[j * n : (j + 1) * n] = S[s]
                    E_rows.append(r)
                    f_rows.append(float(budgets[j] * bs[s]))
    E = np.vstack(E_rows)
    f = np.asarray(f_rows)
    ...
    W = res.x.reshape(K, n)
    metrics = dict(
        sum_weights_per_account=[float(W[j].sum()) for j in range(K)],
