...
A windowed `MeanPredictor` re-reads the whole training window and re-factorizes
it every rebalance. The incremental version instead keeps a **sample pool** of
sufficient statistics updated one sampling period at a time, so the
per-rebalance cost is independent of how far back history reaches.
...
from ._base import LinearParams
...
def solve_standardized(
    ...
    Gram, then solves the normal equations `(ZᵀZ + α n I) β = Zᵀy` — the same
    fit `ridge` reaches by augmented QR, reached instead from moments that can
    ...
    # Pooled mean / population std, with the same constant-column fallback the
    # windowed predictors use.
    x_mean = sx / n
