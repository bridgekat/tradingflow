...
(`.values(start, end)`/`.last()`/`.at(i)`/`len()`/`[-1]`), and clock views for
clock leaves of any rank (a rank-0 pulse answers `if clock:` directly; a clock
array reads as a NumPy bool mask via `.value()`). `timestamp` is TAI
nanoseconds.

`compute` **returns** this generation's output batch, like `Segment::compute`
does: an array pulses the output clock, `None` is quiescence (the values wire
