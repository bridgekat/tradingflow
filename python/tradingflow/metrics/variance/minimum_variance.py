...
class MinimumVariance:
    ...
    Mirrors the `VariancePortfolio` contract: `predictions` is a
    **log-return** covariance matrix (zero-mean lognormal
    specialisation), converted internally to linear-return units before
    solving GMV:
    ...
    **Memory.** Both `predictions` and `target` are `Array` inputs -
    ...
    predictions : Array, shape ``(N, N)``
        ...
        predictor.  Stocks excluded by the variance predictor have NaN
        on the diagonal.
    target : Array, shape ``(N,)``
        Live cross-sectional realized log-returns, produced at every
        tick (e.g. a `Diff(Log(prices))` node).  Non-finite entries
        contribute zero to the portfolio return.
...
def build(**kwargs) -> MinimumVariance:
    """Construct a :class:`MinimumVariance` operator.
    ...
    """
    return MinimumVariance(num_stocks=int(kwargs["num_stocks"]))
