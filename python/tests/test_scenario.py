...
from tradingflow.operators import Add, Multiply, Negate, Scale, Record, Filter, Where
...
class TestScenario:
    def test_run_updates_only_affected_downstream_chain(self) -> None:
        ...
        source_a = ArraySource(timestamps=np.array([ts(1), ts(2)]), values=np.array([1.0, 4.0]), name="a")
        source_b = ArraySource(timestamps=np.array([ts(1)]), values=np.array([2.0]), name="b")
        source_c = ArraySource(timestamps=np.array([ts(1), ts(3)]), values=np.array([10.0, 5.0]), name="c")
    ...
    def test_same_timestamp_updates_are_coalesced(self) -> None:
        ...
        source_a = ArraySource(timestamps=np.array([ts(1), ts(1)]), values=np.array([1.0, 2.0]))
        source_b = ArraySource(timestamps=np.array([ts(1)]), values=np.array([3.0]))
    ...
    def test_run_rejects_decreasing_source_timestamps(self) -> None:
        ...
        with pytest.raises(ValueError, match="non-decreasing"):
            ArraySource(timestamps=np.array([ts(2), ts(1)]), values=np.array([1.0, 2.0]))
    ...
    def test_output_timestamps_strictly_increasing(self) -> None:
        ...
        for i in range(4):
            ...
            sources.append(ArraySource(timestamps=raw_ts, values=values, name=f"src_{i}"))
    ...
    def test_two_random_sources_add_matches_simulation(self) -> None:
        ...
        sc = Scenario()
        a = sc.add_source(
            ArraySource(timestamps=raw_ts_a.astype("datetime64[ns]"), values=vals_a.astype(np.float64), name="a")
        )
        b = sc.add_source(
            ArraySource(timestamps=raw_ts_b.astype("datetime64[ns]"), values=vals_b.astype(np.float64), name="b")
        )
    ...
    def test_interleaved_sources_trigger_output_at_every_timestamp(self) -> None:
        ...
        sc = Scenario()
        a = sc.add_source(ArraySource(timestamps=ts_a, values=vals_a, name="a"))
        b = sc.add_source(ArraySource(timestamps=ts_b, values=vals_b, name="b"))
        result_h = sc.add_operator(Add(a, b))
        result_s = sc.add_operator(Record(result_h))
        sc.run()
    ...
    def test_same_source_added_twice_creates_independent_nodes(self) -> None:
        ...
        source = ArraySource(timestamps=np.array([ts(1), ts(2), ts(3)]), values=np.array([10.0, 20.0, 30.0]))
        sc = Scenario()
    ...
    def test_scenario_run_is_repeatable(self) -> None:
        ...
        for i in range(3):
            ...
            sources.append(ArraySource(timestamps=raw_ts, values=vals, name=f"s{i}"))
    ...
    def test_filter_stops_downstream_propagation(self) -> None:
        ...
        source = ArraySource(timestamps=np.array([ts(1), ts(2), ts(3)]), values=np.array([-1.0, 2.0, -3.0]))
        sc = Scenario()
    ...
    def test_where_replaces_elements(self) -> None:
        ...
        source = ArraySource(timestamps=np.array([ts(1)]), values=np.array([[1.0, 5.0, 2.0]]))
        sc = Scenario()
        s = sc.add_source(source)
    ...
    def test_python_operator_chained_with_native(self) -> None:
        ...
        source = ArraySource(timestamps=np.array([ts(1), ts(2), ts(3), ts(4)]), values=np.array([1.0, 5.0, 2.0, 10.0]))
        sc = Scenario()
        s = sc.add_source(source)
...
class TestMixedSourcesAndOperators:
    """Scenarios with mixed Rust (native) and Python sources/operators."""

    def test_native_and_py_sources_add(self) -> None:
        """Native ArraySource + Python IterSource fed into native Add."""
        sc = Scenario()
        ha = sc.add_source(ArraySource([ts(1), ts(2), ts(3)], [10.0, 20.0, 30.0]))
        hb = sc.add_source(IterSource(
    ...
    def test_native_and_py_sources_interleaved(self) -> None:
        """Native and Python sources at different timestamps merge correctly."""
        sc = Scenario()
    ...
    def test_py_source_native_op_py_op_chain(self) -> None:
        """Python source -> native Scale -> Python Filter -> Record."""
        sc = Scenario()
        h = sc.add_source(IterSource(
        ...
        sc.run()
        # 10,20,30,40,50,60,70 -> filter >=50 -> 50,60,70
        assert list(sc.series_view(hr).values()) == pytest.approx([50.0, 60.0, 70.0])

    def test_multiple_py_sources(self) -> None:
        """Two Python IterSources added together."""
        sc = Scenario()
        ha = sc.add_source(IterSource(
            [(ts(1), 1.0), (ts(2), 2.0)], shape=(), dtype=np.float64,
        ))
        hb = sc.add_source(IterSource(
            [(ts(1), 10.0), (ts(2), 20.0)], shape=(), dtype=np.float64,
        ))
        hc = sc.add_operator(Add(ha, hb))
        hr = sc.add_operator(Record(hc))
        sc.run()
    ...
    def test_py_source_vector_values(self) -> None:
        """Python source with vector values through native operators."""
        sc = Scenario()
        ha = sc.add_source(IterSource(
            [(ts(1), np.array([1.0, 2.0]))], shape=(2,), dtype=np.float64,
        ))
        hb = sc.add_source(IterSource(
            [(ts(1), np.array([10.0, 20.0]))], shape=(2,), dtype=np.float64,
        ))
        hc = sc.add_operator(Add(ha, hb))
        hr = sc.add_operator(Record(hc))
        sc.run()
        np.testing.assert_array_almost_equal(
            sc.series_view(hr).values().flatten(), [11.0, 22.0]
        )
