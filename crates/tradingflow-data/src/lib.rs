...
//! owns its data.
//!
//! The [`ArrayView<'a, T, N>`] type is an `N`-dimensional array of scalars `T`,
//! which borrows its data from elsewhere (with lifetime `'a`).
//!
//! The [`Array::view`] method creates an [`ArrayView`] borrow of its data, and
//! [`ArrayView::to_array`] copies the borrowed data to create an owned
//! [`Array`].
//!
//! # Series
//!
//! The [`Series<T, N>`] type is a time series whose elements are
//! `N`-dimensional arrays of scalars `T`, which owns its data.
//!
//! The [`SeriesView<'a, T, N>`] type is a time series whose elements are
//! `N`-dimensional arrays, which borrows its data from elsewhere
//! (with lifetime `'a`).
//!
//! The [`Series::view`] method creates a [`SeriesView`] borrow of its data,
//! and [`SeriesView::to_series`] copies the borrowed data to create an owned
//! [`Series`].
//!
//! # Timestamps
//!
