...
impl Instant {
    ...
    /// The instant at **UTC midnight** of `days` since 1970-01-01, i.e. the
    /// UTC-day-number → TAI-instant parse.  This is the date convention of
    /// daily bar data: a `date32` column, or a `YYYY-MM-DD` string via
    /// [`days_from_civil`].
    #[inline]
    pub fn from_utc_days(days: i64) -> Self {
        Self::from_utc_nanos(days * NANOS_PER_DAY)
    }

    /// The UTC day number (days since 1970-01-01) containing this instant -
    /// the inverse of [`from_utc_days`](Self::from_utc_days), flooring toward
    /// negative infinity.
    #[inline]
    pub fn to_utc_days(self) -> i64 {
        self.to_utc_nanos().div_euclid(NANOS_PER_DAY)
    }
}

/// Nanoseconds in a calendar day (86 400 SI seconds).
...
/// Days since 1970-01-01 for a proleptic-Gregorian date.  Valid for any date;
/// the result is negative before the epoch.  The inverse of
/// [`civil_from_days`].
