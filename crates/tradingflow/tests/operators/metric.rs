...
fn comp_return_is_the_per_period_geometric_mean() {
    ...
    let out = gated(metric::comp_return(), &path);
...
fn comp_return_of_a_round_trip_is_negative_where_the_mean_is_zero() {
    ...
    let geo = gated(metric::comp_return(), &path);
    let ari = gated(metric::return_mean(), &path);
...
fn return_mean_is_the_running_mean_of_simple_returns() {
    let path = positive_path(12);
    let out = gated(metric::return_mean(), &path);
...
fn log_return_mean_telescopes_to_the_endpoints() {
    let path = positive_path(12);
    let out = gated(metric::log_return_mean(), &path);
    assert_close(
        &out,
...
fn log_return_mean_is_the_log_of_the_geometric_mean() {
    ...
    let log = gated(metric::log_return_mean(), &path);
    let geo = gated(metric::comp_return(), &path);
...
fn return_vol_is_the_population_standard_deviation() {
    let path = positive_path(12);
    let out = gated(metric::return_vol(), &path);
...
fn log_return_vol_is_the_population_deviation_of_log_returns() {
    let path = positive_path(12);
    let out = gated(metric::log_return_vol(), &path);
    assert_close(
        &out,
...
fn return_vol_of_an_exactly_riskless_path_is_zero() {
    let path = geometric_path(100.0, 0.25, 6);
    for (k, &v) in gated(metric::return_vol(), &path).iter().enumerate() {
...
fn return_vol_of_a_nearly_riskless_path_is_never_nan() {
    for &(rate, len) in &[(0.1, 5), (0.1, 9), (0.01, 11), (0.07, 8)] {
        ...
        for (op, out) in [
            ("vol", gated(metric::return_vol(), &path)),
            ("log vol", gated(metric::log_return_vol(), &path)),
        ] {
...
fn return_vol_of_a_single_period_is_zero() {
    assert_eq!(gated(metric::return_vol(), &[100.0, 110.0]), vec![0.0, 0.0]);
}
...
fn return_sharpe_is_mean_over_population_deviation() {
    let path = positive_path(12);
    let out = gated(metric::return_sharpe(), &path);
    let expected = expected_seq(&path, returns, |rs| mean(rs) / pop_std(rs));
    assert_close(&out, &expected, "return_sharpe");

    let means = gated(metric::return_mean(), &path);
    let vols = gated(metric::return_vol(), &path);
    for k in 2..=path.len() {
        assert_close(
            &[out[k - 1]],
            &[means[k - 1] / vols[k - 1]],
...
fn log_return_sharpe_uses_log_returns() {
    let path = positive_path(12);
    let out = gated(metric::log_return_sharpe(), &path);
...
fn return_sharpe_of_a_riskless_path_is_infinite() {
    let up = gated(metric::return_sharpe(), &geometric_path(100.0, 0.25, 5));
    ...
    let down = gated(metric::return_sharpe(), &geometric_path(100.0, -0.25, 5));
...
fn return_sharpe_of_a_flat_path_is_nan() {
    let out = gated(metric::return_sharpe(), &[100.0; 5]);
    assert_eq!(out[0], 0.0);
...
fn drawdown_is_zero_at_every_new_high() {
    let out = ungated(metric::drawdown(), &[100.0, 120.0, 90.0, 150.0]);
...
fn drawdown_measures_against_a_peak_that_never_resets() {
    ...
    let out = ungated(metric::drawdown(), &path);
...
fn return_rejects_a_non_positive_nav() {
    gated(metric::return_mean(), &[100.0, 0.0]);
}
...
fn return_rejects_a_non_finite_nav() {
    gated(metric::return_mean(), &[100.0, f64::NAN]);
}
...
fn return_rejects_a_non_positive_opening_nav() {
    gated(metric::comp_return(), &[0.0, 100.0]);
}
...
fn drawdown_rejects_a_non_positive_nav() {
    ungated(metric::drawdown(), &[100.0, -1.0]);
}
