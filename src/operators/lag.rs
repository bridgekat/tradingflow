...
mod tests {
    ...
    fn lag_basic() {
        ...
        s.push(1, &[10.0]);
        Lag::compute(&mut state, (&s,), &mut out, 1);
        assert!(out.last().unwrap()[0].is_nan());

        s.push(2, &[20.0]);
        Lag::compute(&mut state, (&s,), &mut out, 2);
        assert!(out.last().unwrap()[0].is_nan());

        s.push(3, &[30.0]);
        Lag::compute(&mut state, (&s,), &mut out, 3);
        assert_eq!(out.last().unwrap()[0], 10.0);

        s.push(4, &[40.0]);
        Lag::compute(&mut state, (&s,), &mut out, 4);
    ...
    fn lag_timestamps() {
        ...
        s.push(100, &[1.0]);
        Lag::compute(&mut state, (&s,), &mut out, 100);
        s.push(200, &[2.0]);
        Lag::compute(&mut state, (&s,), &mut out, 200);
