...
impl<T: Copy> Concat<T> {
    ...
    pub fn new(input_shape: &[usize], axis: usize) -> Self {
        if input_shape.is_empty() {
            // Scalar inputs — concat produces a 1-D vector.
            return Self {
                outer_count: 1,
                chunk_size: 1,
    ...
    /// Compute the output shape for a concat operation.
    ///
...
impl<T: Copy> Operator for Concat<T> {
    type Inputs<'a>
        = &'a [&'a Observable<T>]
    where
        Self: 'a;
...
mod tests {
    ...
    fn output_shape_computation() {
        assert_eq!(Concat::<f64>::output_shape(&[], 3, 0), vec![3]);
        assert_eq!(Concat::<f64>::output_shape(&[2], 3, 0), vec![6]);
        assert_eq!(Concat::<f64>::output_shape(&[2, 3], 2, 0), vec![4, 3]);
        assert_eq!(Concat::<f64>::output_shape(&[2, 3], 2, 1), vec![2, 6]);
    }
}
