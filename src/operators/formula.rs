...
//! [`segment!`](flowgraph::segment) formulas read like the formulas themselves.
//!
...
//! [`segment!`](flowgraph::segment) body:
//!
//! ```ignore
//! let clk = sc.time();
//! let signal = sc.push(
...
use flowgraph::typed::{Comp, Fork, Id, Right, ViewPort};
...
pub fn ma_time<T: Scalar + Float, const N: usize>(
    ...
    Comp(
        Record::with_retention(clock.clone(), Retention::duration(window + TIME_MARGIN)),
...
pub fn ema<T: Scalar + Float, const N: usize>(
    ...
    Comp(buffer(clock, window), Ema::new(alpha, window))
}
...
pub fn lag_or<T: Scalar, const N: usize>(
    ...
    Comp(buffer(clock, n), Lag::new(n, fill))
}
...
pub fn growth<T: Scalar + Float, const N: usize>(
    ...
    N,
    Comp<
        Fork<BinaryFn<T, N>, Right<ViewPort<ArrayValue<T, N>>, ViewPort<ArrayValue<T, N>>>>,
        BinaryFn<T, N>,
    >,
> {
    Comp(
        Fork(Id::default(), lag(clock, n)),
