...
mod tests {
    ...
    fn ffill_basic() {
        ...
        a[0] = 10.0;
        ForwardFill::compute(&mut state, &a, &mut out, Instant::from_nanos(1), false);
        assert_eq!(out[0], 10.0);

        a[0] = f64::NAN;
        ForwardFill::compute(&mut state, &a, &mut out, Instant::from_nanos(2), false);
        assert_eq!(out[0], 10.0); // forward-filled

        a[0] = 30.0;
        ForwardFill::compute(&mut state, &a, &mut out, Instant::from_nanos(3), false);
