//! Multiply operator — element-wise multiplication of two `Array<f64>`.
...
mod tests {
    ...
    #[test]
    fn basic() {
        let a = Array::from_vec(&[2], vec![2.0, 3.0]);
        let b = Array::from_vec(&[2], vec![10.0, 100.0]);
        let (mut s, mut o) = Multiply::new().init((&a, &b), Instant::MIN);
        Multiply::compute(&mut s, (&a, &b), &mut o, Instant::from_nanos(1), (false, false));
        assert_eq!(o.as_slice(), &[20.0, 300.0]);
    }
}
