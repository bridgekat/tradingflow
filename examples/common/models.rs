//! Typed constructors for the `tradingflow` Python predictors, portfolios and metrics.
//!
//! Each `tradingflow` operator is reached through [`PyClassOperator`], whose three
...
//! `benchmark_relative_strategy` silently disagreed with `tradingflow`' documented
...
/// `Mode` in `tradingflow.portfolios.mean_variance._modes`.
#[derive(Clone, Copy)]
pub enum Mode {
    MinMeanVariance = 3,
}
...
pub fn ridge_mean(d: Dims, min_periods: i64, alpha: f64) -> MeanPredictor {
    py_class_operator(
        "tradingflow.predictors.mean.incremental_ridge",
        d.params()
...
pub fn linear_regression_mean(d: Dims, min_periods: i64) -> MeanPredictor {
    py_class_operator(
        "tradingflow.predictors.mean.incremental_linear_regression",
...
/// A covariance estimator: a `tradingflow.predictors.variance.*` module plus the
...
impl CovEstimator {
    pub const fn sample(name: &'static str) -> Self {
        Self {
            name,
            module: "tradingflow.predictors.variance.sample",
            target: None,
            mode: None,
        }
    }
    ...
    pub const fn shrinkage(name: &'static str, target: i64) -> Self {
        Self {
            name,
            module: "tradingflow.predictors.variance.shrinkage",
            target: Some(target),
            mode: None,
        }
    }
    ...
    pub const fn rmt(name: &'static str, mode: &'static str) -> Self {
        Self {
            name,
            module: "tradingflow.predictors.variance.rmt",
            target: None,
            mode: Some(mode),
        }
    }
    pub const fn single_index(name: &'static str) -> Self {
        Self {
            name,
            module: "tradingflow.predictors.variance.single_index",
            target: None,
            mode: None,
        }
    }
    ...
    pub fn build(&self, d: Dims, max_periods: i64, min_periods: Option<i64>) -> CovPredictor {
        ...
        py_class_operator(self.module, p, vec![d.num_stocks, d.num_stocks])
    }
}

/// Ledoit-Wolf shrinkage toward the common-covariance target — the default
/// covariance for the mean-variance strategies.
pub fn shrinkage_cov(d: Dims, max_periods: i64, min_periods: i64) -> CovPredictor {
    CovEstimator::shrinkage("shrinkage_comm_cov", 1).build(d, max_periods, Some(min_periods))
}
...
pub fn rank_linear(num_stocks: usize, top_fraction: f64) -> MeanPortfolio {
    py_class_operator(
        "tradingflow.portfolios.mean.rank_linear",
        PyParams::new()
...
pub fn markowitz(
    ...
    py_class_operator(
        "tradingflow.portfolios.mean_variance.markowitz",
        PyParams::new()
            .int("num_stocks", num_stocks as i64)
...
pub fn benchmark_relative(
    ...
    py_class_operator(
        "tradingflow.portfolios.mean_variance.benchmark_relative",
        PyParams::new()
            .int("num_stocks", num_stocks as i64)
            .int("max_universe_size", universe_size as i64)
            .float("bound", bound)
...
pub fn regression_coefficients(
    ...
    py_class_operator(
        "tradingflow.metrics.mean.regression_coefficients",
        PyParams::new()
...
pub fn minimum_variance(num_stocks: usize) -> MinimumVariance {
    py_class_operator(
        "tradingflow.metrics.variance.minimum_variance",
...
pub fn information_coefficient(num_stocks: usize) -> InformationCoefficient {
    py_class_operator(
        "tradingflow.metrics.mean.information_coefficient",
        PyParams::new().int("num_stocks", num_stocks as i64),
        vec![],
    )
}
