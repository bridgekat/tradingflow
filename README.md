...
An [observable value](src/tradingflow/observable.py) is a piece of data that can be updated at strictly increasing `np.datetime64` timestamps.
...
A [time series](src/tradingflow/series.py) stores a sequence of uniformly typed elements indexed by strictly increasing `np.datetime64` timestamps, supporting integer indexing, slicing, timestamp lookups, and amortized O(1) appends. Any observable value can be *materialized* into a time series.
