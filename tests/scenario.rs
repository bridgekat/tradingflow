//! Integration tests for the strategy engine: TradingFlow sources + operators
...
async fn on_stable_per_batch() {
    ...
    let mut session = sc.build();
    assert_eq!(session.estimated_event_count(), Some(3));
    let mut batches = Vec::new();
    session
        .run(|s, ts| batches.push((ts.as_nanos(), s.events())))
        .await;
